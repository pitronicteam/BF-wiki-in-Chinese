...
| `BADRX`（1） | 您的接收器刚刚从接收器失控保护中恢复，但解锁开关仍处于激活状态 | 4 | 4 | 4 | 4 | 4 | 4 | 将解锁开关拨动至上锁档位 |
| `BOXFAILSAFE` | “ 失控保护”开关已激活 | 5 | 5 | 5 | 5 | 5 | 5 | 请查阅`FAILSAFE` |
| `RUNAWAY` | 防止跑道起飞 |  | 6 | 6 | 6 | 6 | 6 | 上锁以清除此情况 |
| `CRASH` | 炸机恢复已触发 |  |  |  |  | 7 | 7 | 上锁以清除此情况 |
| `THROTTLE` | 油门通道过高 | 6 | 7 | 7 | 7 | 8 | 8 | 将油门降低至`min_check`以下 |
| `ANGLE` | 飞行器不（足够）水平 | 7 | 8 | 8 | 8 | 9 | 9 | 将飞船水平调整到`small_angle`以下（默认为25） |
| `BOOTGRACE` | 开机后解锁太快 | 8 | 9 | 9 | 10 | 10 | 10 | 等`pwr_on_arm_grace`（默认为5）秒 |
...
| `CMS` | CMS（配置菜单）处于活动状态-通过OSD或其他显示 | 13 | 14 | 14 | 14 | 15 | 15 | 退出CMS（或OSD菜单） |
| `OSD` | OSD菜单处于活动状态 | 14 | 15 | 16 |  |  |  | 退出OSD菜单 |
| `BST` | 黑羊遥测设备（例如TBS Core Pro）已撤防并无法布防 | 15 | 16 | 16 | 15 | 16 | 16 | 请参阅您的硬件手册 |
| `MSP` | MSP连接处于活动状态，可能是已经连接了Betaflight配置程序 | 16 | 17 | 17 | 16 | 17 | 17 | 终止与Betaflight配置程序的连接（断开连接） |
